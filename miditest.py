...
for dev_id in range(pygame.midi.get_count()):
    ...
    if is_output:
        print(f"  {dev_id}: {name.decode(errors='ignore')}")

# ② デバイスを自動選択する（例：GS Wavetable Synthを優先）
midi_out = None
selected_name = None
for dev_id in range(pygame.midi.get_count()):
    interf, name, is_input, is_output, opened = pygame.midi.get_device_info(dev_id)
    ...
    if 'GS Wavetable Synth' in name_str:
        try:
            midi_out = pygame.midi.Output(dev_id)
            selected_name = name_str
            break
        except:
            pass
...
if midi_out is None:
    for dev_id in range(pygame.midi.get_count()):
        ...
        try:
            midi_out = pygame.midi.Output(dev_id)
            selected_name = name.decode(errors='ignore')
            break
        except:
            pass
...
# ④ 最終的に使われたデバイス名を表示
if midi_out:
    print(f"Using MIDI Output Device: {selected_name}")
else:
    raise RuntimeError("No usable MIDI output device found.")

# --- 以下、midi_out.write_short() 等でメッセージ送信 ---
